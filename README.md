...
> 轻量探索模式 — 用最少 token 完成代码搜索和调研。核心理念：只读搜索不需要加载全部工具和项目规则。适用于：(1) 搜索代码库找文件/函数 (2) 调研项目架构 (3) 多文件分析 (4) 回答"代码里哪里有 X"类问题。灵感源自 Claude Code Explore Agent 设计。
...
```bash
openclaw skill install github:Arxchibobo/lightweight-explorer
```
...
git clone https://github.com/Arxchibobo/lightweight-explorer.git lightweight-explorer
```
